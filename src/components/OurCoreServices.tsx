...
import shopiy from "../../public/electrical4.jpeg";
...
const services = [
	{
		...
		description:
			"Set up your online business with the best eCommerce platform, Shopify. As the top Shopify developers in Pakistan, we ensure the success of your online store.",
	},
	{
		image: img1,
		...
		description:
			"We are the leading web development company in Pakistan that provides end-to-end custom web app development solutions to solve your unique business challenges.",
	},
	{
		image: mobileApp,
		alt: "TOXIN REMOVAL",
...
const ServiceItem = ({ image, alt, title, description }: any) => (
	...
	>
		...
		></div>
		<div
			className="absolute inset-0 flex translate-y-[60%] flex-col items-start justify-center 
		...
		>
			<div className="flex items-center mb-2 group-hover:opacity-100">
				<h1 className="font-['Calibri'] text-2xl text-white ml-2">{title}</h1>
			</div>
	...
	return (
		<>
			...
			<div className="flex mb-5 lg:-mt-16 sm:mt-5 md:mt-5 items-center justify-center">
				<Link href="/services">
					<button
						className="relative border-2 border-[#fcb040] bg-transparent py-2.5 px-5 font-medium uppercase 
